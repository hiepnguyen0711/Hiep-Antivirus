...
class func_index
{
  ...
  <path d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288L8 2.223l1.847 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.565.565 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z"/>
    ...
    // ✅ Kiểm tra bảng có tồn tại trong cơ sở dữ liệu (dùng PDO)
    public function tableExists($tableName)
    {
        try {
            $tableName = str_replace('#_', $this->refix, $tableName);
    ...
    // ✅ Kiểm tra cột có tồn tại trong bảng (dùng PDO)
...
class HiepSecurity
{
     ...
     */
    public function validateFileUpload($file, $allowed_types = array(), $max_size = 5242880)
    {
        $errors = array();
        ...
        if (function_exists('check_shell') && check_shell($content) === '') {
            $errors[] = 'Malicious content detected';
            $this->logSecurity('Malicious file upload attempt: ' . $file['name'], 'CRITICAL');
        }
    ...
    {
        $attempts_file = dirname($this->log_file) . '/login_attempts.json';
        $attempts = array();
        ...
        if (isset($attempts[$ip])) {
            $attempts[$ip] = array_filter($attempts[$ip], function($timestamp) use ($current_time) {
                return ($current_time - $timestamp) < $this->lockout_duration;
            });
        }

        // Check if IP is locked out
        if (isset($attempts[$ip]) && count($attempts[$ip]) >= $this->max_login_attempts) {
            return false;
        }
    ...
    public function recordFailedLogin($ip)
    {
        ...
        $this->logSecurity('Failed login attempt from IP: ' . $ip, 'WARNING');
    }
